...
import southKoreaFlyer from "../../assets/gallery/south-korea-flyer.png";
import filipinoHandwearer from "../../assets/gallery/filipino-handwearer.png";
...
export function Gallery() {
  ...
  const [galleryFilter, setGalleryFilter] = useState<"all" | "web" | "graphic">("all");
  const [galleryStats, setGalleryStats] = useState([
    ...
    {
      src: southKoreaFlyer,
      alt: "South Korea Group Tour Flyer",
      category: "Graphic Design",
      views: 872,
      likes: 61,
      isLiked: false,
    ...
    {
      src: filipinoHandwearer,
      alt: "Filipino Handwearer Product Advertisement",
      category: "Graphic Design",
      views: 934,
      likes: 74,
      isLiked: false,
  ...
  const visibleGalleryItems = galleryStats
    .map((item, index) => ({ item, index }))
    .filter(({ item }) => {
      if (galleryFilter === "web") return item.isWebProject;
      if (galleryFilter === "graphic") return !item.isWebProject;
      return true;
    });
  ...
  return (
    <section id="gallery" className="py-20 bg-muted/30">
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="mb-8 flex flex-wrap justify-center gap-2" role="group" aria-label="Gallery filters">
          {[
            ...
            <button
              key={filter.value}
              type="button"
              onClick={() => setGalleryFilter(filter.value as "all" | "web" | "graphic")}
